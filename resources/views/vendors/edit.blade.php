...
<div class="container">
    ...
    <form action="{{ route('vendors.update', \$vendor->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="products_services_supplied" name="products_services_supplied" rows="3">{{ old('products_services_supplied', \$vendor->products_services_supplied) }}</textarea>
        </div>

        <div class="form-group">
            <label for="sla_terms">SLA Terms</label>
            <textarea class="form-control" id="sla_terms" name="sla_terms" rows="3">{{ old('sla_terms', \$vendor->sla_terms) }}</textarea>
        </div>

        <div class="form-group">
            <label for="escalation_matrix">Escalation Matrix</label>
            <textarea class="form-control" id="escalation_matrix" name="escalation_matrix" rows="3">{{ old('escalation_matrix', \$vendor->escalation_matrix) }}</textarea>
        </div>
